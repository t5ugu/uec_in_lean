...
def fst {C : Type u₀} {D : Type u₁} [Category.{v₀} C] [Category.{v₁} D] : (C × D) ⥤ C where
  obj X := X.1
  map f := f.1
  map_id X := by simp
  map_comp f g := by simp

def snd {C : Type u₀} {D : Type u₁} [Category.{v₀} C] [Category.{v₁} D] : (C × D) ⥤ D where
  obj X := X.2
  map f := f.2
  map_id X := by simp
  map_comp f g := by simp

def swap (C : Type u₀) (D : Type u₁) [Category.{v₀} C] [Category.{v₁} D] : (C × D) ⥤ (D × C) where
  obj X := (X.2, X.1)
  map f := (f.2, f.1)
  map_id X := by simp
  map_comp f g := by simp
